...
from database import get_last_messages, save_post, post_exists
...
async def run_bot():
    ...
    try:
        ...
        # Проверяем, не отправляли ли уже такой пост
        if post_exists(post):
            logger.info("⚠️ Похожий пост уже был отправлен сегодня, пропускаем")
            return "⚠️ Похожий пост уже был отправлен сегодня"
